...
def pseudo_sort(st): 
    st = re.sub(r"[',','!',':','.',';','?']", "", st)
    lst_upper = [word for word in st.split() if word[0].isupper()]
    lst_lower = [word for word in st.split() if word[0].islower()]
    return ' '.join(sorted(lst_lower)+sorted(lst_upper)[::-1])
